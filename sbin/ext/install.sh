...
if [ ! -f /system/.hydra/cwmmanager3-installed ];
then
  if [ "$payload_extracted" == "0" ];then
    extract_payload
  fi
  ...
  mkdir /system/.hydra
  chmod 755 /system/.hydra
  echo 1 > /system/.hydra/cwmmanager3-installed
fi
...
#if [ "${romtype}a" == "0a" ];
#then
#if [ ! -f /system/.hydra/liblights-installed ];then
  ...
  echo 1 > /system/.hydra/liblights-installed
#fi
#fi
